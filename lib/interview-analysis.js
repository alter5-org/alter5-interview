...
// See lib/cv-analysis.js — retired model, env-overridable.
...
${stripXmlBreakout(String(summaryText).slice(0, 50000))}
  ...
  try {
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: MODEL,
        // Report is ~1500 tokens; leave room for thinking. Effort stays low
        // so the inline grade finishes inside submit-interview's 45 s abort.
        max_tokens: 8000,
