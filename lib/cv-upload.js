...
const { getPositionById, getPositionBySlug, routeCvScore, reviewThreshold } = require('./positions');
...
async function processCvUpload({
  ...
  positionSlug = null,      // alternative way to pin position (e.g. email agent env slug)
  uploaderNote = null,
  autoInvite = false,
  ip = null,
  userAgent = null,
  ...
  // prefetch and the final scoring. Caller passes id OR slug. There is NO
  // implicit default: with several open positions a silent fallback would
  // grade a candidate against the wrong prompt without any error.
  let position = null;
  ...
  if (!position) {
    return { ok: false, status: 400, error: 'position_required' };
  }
  ...
  if (existing) {
    appRow = existing;
    // If this candidate already exists under a different position, the
    // DB record is the source of truth: we do NOT silently re-route them
    // to the position the uploader picked. Otherwise the admin would see
    // `position=A` while the analysis used position B's prompt. Re-pin
    // position + rerun analysis against the correct prompt so the
    // persisted score matches the application's actual position.
    if (existing.position_id && existing.position_id !== position.id) {
      const existingPos = await getPositionById(existing.position_id);
      if (existingPos && existingPos.status === 'active') {
        position = existingPos;
        // The email-extraction prefetch (if any) was run against the
        // uploader's chosen prompt — drop it so the final analyze step
        // re-runs against the candidate's real position prompt.
        analysisPrefetched = null;
      }
      // If the existing position is inactive/archived we stick with the
      // uploader's chosen position; the application was already stale.
    }
  ...
  if (autoInvite) {
    ...
    nextStatus = routeCvScore(analysis.score, position);
  }
