...
// Fallback prompt — used only if the caller doesn't pass `systemPrompt`.
// The current production flow always loads the prompt from
// positions.cv_analysis_prompt and passes it in. This default exists so a
// position with a missing/empty prompt still produces a sensible analysis
// instead of a 500.
const DEFAULT_SYSTEM_PROMPT = `Eres un recruiter senior especializado en perfiles tech de nivel C-level y arquitectura de software.
...
async function analyzeCv({ fileBase64, filename, systemPrompt = null }) {
